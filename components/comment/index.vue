		...
		<view class="tab-bar">
			...
			<view class="mview">
				...
				<view class="mbtn" @tap="sendLove">
					<image class="micon" v-if="type=='news'"
						:src=" oneInfoClone.has_like ? '/static/lover.png' : '/static/love.png' " mode="aspectFit" />
					<text class="mtext"
						v-if="type=='news' && oneInfoClone.digg_count>0 ">{{ oneInfoClone.digg_count }}</text>
					<text class="mtext" v-if="type=='info' && oneInfoClone.digg_count<=0 ">点赞</text>

					<image class="micon" v-if="type=='feeds'"
						:src=" oneInfoClone.has_like ? '/static/lover.png' : '/static/love.png' " mode="aspectFit" />
					<text class="mtext"
						v-if="type=='feeds' && oneInfoClone.like_count>0 ">{{ oneInfoClone.like_count }}</text>
					<text class="mtext" v-if="type=='feeds' && oneInfoClone.like_count<=0 ">点赞</text>
				</view>
				...
				<view class="mbtn" @tap="gotoComment">
					...
					<text class="mtext"
						v-if="type=='news' && oneInfoClone.comment_count>0 ">{{ oneInfoClone.comment_count }}</text>
					<text class="mtext" v-if="type=='news' && oneInfoClone.comment_count<=0 ">评论</text>

					<text class="mtext"
						v-if="type=='feeds' && oneInfoClone.feed_comment_count>0 ">{{ oneInfoClone.feed_comment_count }}</text>
					<text class="mtext" v-if="type=='feeds' && oneInfoClone.feed_comment_count<=0 ">评论</text>
				</view>
		...
		methods: {
			...
			getInput(e) {
				if (e.detail.cursor > 0) {
					...
					this.commentCon = e.detail.value
				}
			},
			gotoComment() {
